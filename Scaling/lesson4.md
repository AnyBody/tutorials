...
# Lesson 4: Personalizing Individual Segments Based on Geometric Data from Medical Images

This tutorial presumes that you have completed the first tree lessons on
...
the bone surface. As opposed to the simple scaling laws explained in the first
tree lessons, this lesson is rather methodological than conceptual and provides
...
Scaling schemes described in the first tree lessons are based on anthropometric
...
these schemes are used quite often. However, a natural next step would be to
improve the precision of a model by utilizing subject-specific geometry
available from the medical images. Medical images contain more subject-specific
information about the bone shapes and local deformities that cannot be handled
by the anthropometric regression equations.

The simplest inclusion of the subject-specific bone shape from medical image
data is to find the affine (linear) transformation that fits a number of
corresponding points that are selected on the source and the target geometries.
These points could be fitted e.g. in a least-squares manner. This approach is
similar to utilizing external body measurements as it relies on a linear
transform. However, it is less dependent on the bone orientation and prior
knowledge of dimensions to be measured. For example, you can locate any two
points on source and target surface consistently without thinking of how a
segment length was measured.
...
{download}`lesson4a.main.any <Downloads/lesson4a.Main.any>`. This file contains
...
of this transform is described in an {doc}`appendix tutorial <lesson4_appendix>`.
...
You can find the model containing all steps so far in this lesson here: 
{download}`lesson4b.Main.any <Downloads/lesson4b.Main.any>`.
...
:alt: Targe, linear, and RBF scaling
:align: center
:width: 60%
```

This code constructs a transform, which deforms the source geometry into the
target one using the thin-plate interpolation method and minimizes the distance
between the selected key points (landmarks). This can be used when certain
muscle attachment areas/points need to be scaled. Using this allows us improving
the model by making some local features more accurate for the sensitive
analyses. Target bone is color-coded with the yellow color, initial linear
scaling is grey, RBF-scaled bone is red. Please note that `MyTransform` object
was included as a pre-transform as a rough scaling preceding the nonlinear RBF
function, and it will be applied to the source entities, i.e. achieving the
result of the previous step. If this linear pre-transform step was not included
in this case, the RBF transformation would perform poorly.

:::{tip}
Mouse-over in the Model View helps to see the name of the object.
:::

However, it is still possible to improve the fitting of the femur surfaces and,
thus, improve the accuracy of the model. Looking at the Model View you can
notice that the red and yellow surfaces are slightly different, e.g. at the
femoral head region. This is caused by the nature of the interpolation and a low
number of control points. The following section will describe how to utilize
surface information for the construction of an improved scaling law.

## Incorporating Surface-Based Nonlinearities into the Scaling Function

In this section, next improvement to the morphing is added by utilizing surface
information. The surfaces will be requested to be morphed into each other, which
will at the same time deform all related soft tissue attachment points
accordingly. The `AnyFunTransform3DSTL` class is used for this purpose. This
class constructs an RBF transformation similarly to the `AnyFunTransform3DRBF`
by using either 1) corresponding vertices on the STL surfaces or 2) seeding a
number of vertices on one surface and finding a matching closest point on the
second.
...
More details about how to create this kind of transforms are described in 
{doc}`appendix tutorial <lesson4_appendix>`. However, for this example, the
...
the model with all modifications {download}`here <Downloads/lesson4c.Main.any>`:
...
When observing the yellow target surface alongside the blue STL-transformed
surface, it becomes evident that the two surfaces align exceptionally well. This
indicates that the source surface (originally represented in bone color) has
been successfully transformed into the blue surface, which now closely conforms
to the target surface (yellow). This transformation demonstrates a high degree
of accuracy in morphing the source geometry to match the target geometry.

If we just look at the yellow target surface alongside the blue STL-transformed
surface, we can see that the surfaces now match each other very well. This
indicates that the source surface (bone color) has been successfully transformed
into the blue surface, which now closely matches the target surface (yellow).
That means the subject-specificity will be taken into account in the
inverse dynamics simulation. The final model can be downloaded
{download}`here<Downloads/lesson4d.Main.any>`.

```{toctree}
:hidden: true

 Lesson 4 appendix <lesson4_appendix>
```

Finally, the only thing left is to include this scaling function into an
actual model. {doc}`Lesson 5 <lesson5>` describes how this can be
done.