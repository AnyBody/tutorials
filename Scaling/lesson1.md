...
available. As a starting point for this tutorial, please find this model from
the AMMR:
...
To use the StandardScaling law you do not need to do anything at all; however,
for demonstration purposes the scaling law configuration parameter (BM_SCALING)
...
We can now easily personalize the model using anthropometric measurements.
...
implementation allows you to overwrite only the ones you need. For example, if you
...
`Main.HumanModel.Anthropometrics.BodyMass`. The default value is 75 kg.
...
results. By browsing your way to the `MaxMuscleActivity` in the folder `Main.Study.Output` 
...
the fat percentage of the body according to Frankenfield et al.[^cite_BMI_Scale]
...
[^cite_BMI_Scale]: Frankenfield, D. C.; Rowe, W. A.; Cooney, R. N.; Smith, J. S. & Becker, D. (2001): Limits of body mass index to detect obesity and predict body composition, Nutrition 17(1), 26-30.
