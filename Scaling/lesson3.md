...
ones by the input parameters and, therefore, allows another set of
...
:alt: Head Markers Front View
:width: 45%
```

```{image} _static/lesson3/HeadMarkersLateralView.jpg
:alt: Head Markers Lateral View
:width: 45%
```
...
:alt: Head dimensions in Model Tree
:align: center
```
...
This concludes the lessons on the six built-in scaling laws available in
AnyBody. {doc}`Lesson 4 <lesson4>` will introduce an advanced procedure of model
personalization and scaling based on patient-specific geometry data from CT scans.