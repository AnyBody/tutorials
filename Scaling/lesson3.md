...
{doc}`Lesson 1: Joint to joint scaling methods <lesson1>` and 
{doc}`Lesson 2: Scaling based on External Body Measurements <lesson2>`. 
...
computation of these distances and we just need to check the original values in the Model
Tree:
...
personalization and scaling based on geometry data from CT scans.
