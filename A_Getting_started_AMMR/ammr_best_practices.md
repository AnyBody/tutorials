...
```{image} best-practices-libdef.png
...
You can either {doc}`create a model from scratch</A_Getting_started_anyscript/index>` or 
use one of the [application examples](https://anyscript.org/ammr/Applications/index.html)
from the AMMR as a starting point. In either case, the advice is to keep your
demo AMMR as clean as possible. **Please select another working directory on your**
**machine where you can start a new model or copy the folder of the application**
**example that serves you the most, and then write all your code in this new**
**location.**
...
Alternatively, instead of writing out the full path, you can also write the 
following line to correcty refer to the libdef file,
...
configurations through the [body model (BM) statements](https://anyscript.org/ammr/bm_config/index.html).
...
- For some modifications, you must make changes to the body model files. Let's
  ...
  The best way to do this is to {ref}`install <SettingUpAMMR>` a clean 
